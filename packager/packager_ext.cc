...
shaka::Packager* AsPackager(void* foo) {
  return reinterpret_cast<shaka::Packager*>(foo);
}
...
int Packager_Initialize(void* foo, const char* json_args) {
  ...
  std::optional<shaka::PackagingParams> packaging_params =
      shaka::main_func::PackagerParamsFromJSON(json_args);
  if (!packaging_params)
    return shaka::main_func::kArgumentValidationFailed;
  std::optional<std::vector<shaka::StreamDescriptor>> streams =
      shaka::main_func::PackagerStreamsFromJSON(json_args);
  if (!streams || streams.value().size() == 0)
    return shaka::main_func::kArgumentValidationFailed;
  shaka::Status status = AsPackager(foo)->shaka::Packager::Initialize(
      packaging_params.value(), streams.value());
